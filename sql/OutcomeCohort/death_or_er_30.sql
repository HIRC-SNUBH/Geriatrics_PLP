...
DELETE FROM PUBLIC.cohort WHERE COHORT_DEFINITION_ID = 9101;
...
SELECT 9101                  AS COHORT_DEFINITION_ID
  ...
  FROM 
      (SELECT 9101 AS COHORT_DEFINITION_ID
       ...
       SELECT 9101 AS COHORT_DEFINITION_ID
           , T1.PERSON_ID AS SUBJECT_ID
           , T2.COHORT_START_DATE+29 AS COHORT_START_DATE
           , T2.COHORT_START_DATE+30 AS COHORT_END_DATE
       FROM cdm.VISIT_OCCURRENCE T1
           JOIN (SELECT * FROM cohort T2 WHERE T2.COHORT_DEFINITION_ID = 4274) T2
               ON T1.PERSON_ID = T2.SUBJECT_ID AND T1.VISIT_START_DATE  BETWEEN T2.COHORT_END_DATE AND T2.COHORT_END_DATE + 30 
